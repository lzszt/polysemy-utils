...
data Time r a where
  ...
  GetTimeZone :: UTCTime -> Time r TimeZone
...
runTime =
  interpret $ \case
    ...
    GetTimeZone t -> embed $ Data.Time.getTimeZone t
...
runFixedTime (fixedTime, fixedTimeZone) =
  interpret $ \case
    ...
    GetTimeZone _ -> pure fixedTimeZone
